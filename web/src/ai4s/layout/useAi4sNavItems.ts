...
export const AI4S_NAV_ITEMS = [
  ...
  { labelKey: 'sidebar.items.people', href: '/users', match: ['/users', '/roles', '/project/roles'], icon: IconUsersGroup },
...
export function useAi4sNavItems() {
  ...
  // issue #69 P3：按 routeConfigs mode:'hidden' 语义过滤无权限入口（判定落在 href 上，与 RouteGuard 一致；
  // /project/users 已删，人员入口不再做权限回落——无系统级 read_users 即整项隐藏）
  return AI4S_NAV_ITEMS.filter((item) => {
